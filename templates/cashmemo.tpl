        ...
        <div class="paging">
            ...
            {if $ordercount >10}
                <div class="pagination">
                    <ul>
                        {if ($page-1) > 0}
                            <li><a href="{$FRONTEND}cashmemo.php?page={$page-1}">«</a></li>
                        {/if}
                        {if ($page-2)>0}
                            <li><a href="{$FRONTEND}cashmemo.php?page={$page-2}">{$page-2}</a></li>
                        {/if}
                        {if ($page-1)>0}
                            <li><a href="{$FRONTEND}cashmemo.php?page={$page-1}">{$page-1}</a></li>
                        {/if}
                        ...
                        {if ($page+1)<=($pagecount) && ($page+2)<=($pagecount)}
                            {if ($page+2) < $pagecount }
                                <li><a href="{$FRONTEND}cashmemo.php?page={$page+2}">»</a></li>
                            {/if}
                        {/if}
                        <li> of <a href="{$FRONTEND}cashmemo.php?page={$pagecount}">{$pagecount}</a></li>
                    </ul>
                </div>
            {/if}
        </div>
