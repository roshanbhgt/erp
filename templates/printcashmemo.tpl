<div style="width:800px">
    ...
    <div style="float: right; font-size: 11px; margin-top: 6px; color: #666;width: 100%;padding-bottom: 15px;text-align:center;">
        <h2 class="dashboard">Cash Memo Invoice #{$order.chalan_no}</h2>
    </div>
    <div id="cart">
        {if count($customer)>0}
    ...
    {/if}<br/>
    <div style="width:100%;margin-bottom: 15px;text-align: left;float:left;line-height: 24px;" class="std billing">
        <h3>Payment Method</h3>
         <ul>
            <li><input type="checkbox" value="cash" name="payment" {if $order.paymentmethod == 'cash'} checked='true' {/if} />&nbsp;&nbsp;Cash</li>
            <li><input type="checkbox" value="cheque" name="payment" {if $order.paymentmethod == 'cheque'} checked='true' {/if} >&nbsp;&nbsp;Cheque</li>
         </ul><br/><br/>
        <strong>Payment Date</strong> : {$order.markpaidat}
    </div>
        ...
        <table id="item-table" border="1"  align="left">
            ...
            <tfoot>
                ...
                <tr>
                    ...
                    <td style="width: 125px;text-align: center">&#8377;&nbsp;{$order.grandtotal|ceil}</td>
                </tr>
            </tfoot>
        </table>
        {else}
